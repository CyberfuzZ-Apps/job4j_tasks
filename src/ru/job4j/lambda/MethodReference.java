...
в функциональном интерфейсе и вызываемом методе.
Сигнатура это возвращаемый тип и входные параметры.
...
Consumer<String> lambda = (value) -> System.out.prinln(value); // запись через лямбда
...
К статическим методам можно получить доступ через имя класса, как например,
в последнем примере. К не статическим методам можно получить доступ как через объект класса,

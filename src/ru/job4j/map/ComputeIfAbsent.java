...
В интерфейсе Map<K, V> определен особый метод, который позволяет вычислить
значение в том случае, если такого ключа в отображении нет или значение
ассоциированное с ключом равно null. Посмотрим как определен этот метод:

V computeIfAbsent(K key, Function<? super K,? extends V> mappingFunction) -
если в отображении заданный ключ key еще не связан со значением
(или значение равно null), будет выполнено вычисление значение в соответствии
с использованием заданной функции mappingFunction и это значение будет введено в отображение.
...
Необходимо реализовать метод, который произведет ассоциацию id и name
пользователя в отображении. При этом:
...
public class ComputeIfAbsent {
    public static Map<Integer, String> collectData(Map<Integer, String> names, List<User> users) {
        for (User user : users) {
            names.computeIfPresent(user.id, (key, value) -> user.name);
            names.computeIfAbsent(user.id, key -> user.name);
        }
        return names;
    }

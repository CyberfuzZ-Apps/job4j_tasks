...
Если необходимо объединить значение в Map с другим, то для этой задачи можно
воспользоваться методов merge():
...
// заменяем значением новой строки, пары, которая есть в отображении
System.out.println(map.get(1));
// будет выведено Bill Clinton
map.merge(2, "Trump", (oldV, newV) -> oldV + " " + newV);
// если такого ключа нет в отображении - пара будет просто создана
System.out.println(map.get(2));
// будет выведено Trump
...
так долго и ранее не было предусмотрено, что помимо имени необходимо хранить еще и фамилию.
Необходимо обновить информацию - если такой пользователь уже существует (проверять по ключу) -
то необходимо к имени добавить фамилию, если нет - то сначала добавить пользователя -
ключ id, значение - имя пользователя, а потом только обновить и добавить фамилию.
...
public class Merge {
    public static Map<Integer, String> collectData(Map<Integer, String> names, List<User> users) {
        for (User user : users) {
            ...
            names.merge(user.id, user.name, (oldV, newV) -> oldV + " " + user.surname);
        }
        return names;
    }

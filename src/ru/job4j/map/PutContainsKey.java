...
который представляет собой отображение, в котором каждому значению K (ключ)
соответствует значение V(value).
...
V put(K key, V value) - помещает в коллекцию новый объект
с ключом key и значением value. Если в коллекции уже есть объект с подобным ключом,
то он перезаписывается. Если ключа еще не было в коллекции,
то метод возвращает null, иначе - предыдущее значение
...
поэтому если Вы используете свои реализации моделей данных -
эти методы будет необходимо переопределить.

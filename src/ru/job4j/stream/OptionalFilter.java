...
В Optional есть метод, который иногда полезен. Это метод filter(),
он возвращает Optional из значения, если значение удовлетворяет условию,
...
Они решили придти в отдел организации, который отвечает за льготы и узнать
...
public class OptionalFilter {
    ...
    public static List<Child> defineChildren(List<Worker> workers, String passport) {
        ...
        if (worker.isEmpty() || worker.get().children.size() <= 2) {
            return List.of();
        }

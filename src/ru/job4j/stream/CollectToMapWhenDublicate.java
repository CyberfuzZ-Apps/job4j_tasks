...
/*
2.8. Сборка элементов. Сборка в Map, когда есть дубликаты
Метод, который был показан ранее нельзя использовать, если стрим содержит дубликаты.
Для того чтобы собрать данные в мапу в этом случае, мы можем использовать методы
collect() и Collectors.toMap(), но перегруженный. Сборка идет в HashMap

Например,

Map<Integer, Integer> result = List.of(1, 2, 1).stream()
        .collect(Collectors.toMap(k -> k, v -> v, (prev, curr) -> prev));
...
В примере, ключом и значением будет сам элемент, а в случае дубликата мы указываем,
...
public class CollectToMapWhenDublicate {
    public static Map<Integer, Integer> collect(Stream<Integer> data) {
        return data
                .collect(Collectors.toMap(k -> k, v -> v * v, (prev, curr) -> prev));
    }
}

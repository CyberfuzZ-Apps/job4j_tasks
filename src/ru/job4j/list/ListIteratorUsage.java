...
который возвращает объект ListIterator<E>  и позволяет обойти все элементы коллекции:
...
Вы скажете - у нас уже есть Iterator<E>, для чего нам нужен еще один?
Ответ прост - названия у них похожи, но работают они по-разному,
поэтому укажем важные различия и каким пользоваться - выбор за Вами.
...
2. ListIterator позволяет перебирать список в обоих направлениях,
Iterator<E> только в прямом порядке.
...
В дополнение к методам, которые были определены в Iterator<E>, в ListIterator добавлены
еще несколько:
